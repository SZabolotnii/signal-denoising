...
Кожен приклад зберігається у трьох версіях:
...
Рядок `i` у всіх трьох масивах відповідає одному сигналу при одному SNR.
...
У реальних системах виникає від теплового шуму електронних компонентів.

### Негауссовий

| Тип | Спектр | Фізичний прототип |
|---|---|---|
| `polygauss` | Рівний | Нелінійності підсилювача, суперпозиція кількох незалежних джерел шуму |
| `polygauss_nonstationary` | Рівний, нестаціонарний | Мінливе радіосередовище: характеристики шуму змінюються з часом |
...
| Файл | Зміст |
|---|---|
| `plot1_sk_scatter.png` | Хмара точок у площині (γ₃, γ₄); кожна точка - один сигнал |
| `plot2_sk_heatmap.png` | Теплова карта щільності у площині S-K (лог-шкала) |
| `plot3_cumulant_histograms.png` | Гістограми розподілу γ₃...γₙ по всьому датасету |
| `plot4_cumulant_scatter_matrix.png` | Попарні scatter-плоти всіх кумулянтів |
| `plot5_parameter_drift.png` | Дрейф γₙ(t) і σ²(t) всередині окремих сигналів |
| `evaluation_report.txt` | Dataset Card: статистика кумулянтів, покриття, конфіг генерації |
...
Точки з великим γ₄ відповідають сигналам з важкими хвостами або імпульсними компонентами.

**plot2 - S-K Heatmap**

Та сама площина, але колір клітинки кодує кількість сигналів у ній (логарифмічна шкала).
...
Кожна кольорова лінія - окремий сигнал з датасету, розбитий на sub-windows по часу.
Три панелі: γ₃(t), γ₄(t), ..., γₙ(t) і σ²(t) зверху вниз.
...
впродовж одного блоку сигналу. Для `polygauss_nonstationary` очікується
